...
def wrap_multi_agent(env_class):
    """Factory function to wrap an environment class with MultiAgentWrapper."""
    class WrappedEnv(MultiAgentWrapper):
        def __init__(self, *args, **kwargs):
            env = env_class(*args, **kwargs)
            super().__init__(env)
    WrappedEnv.__name__ = f"Wrapped{env_class.__name__}"
    WrappedEnv.__qualname__ = f"Wrapped{env_class.__qualname__}"
    return WrappedEnv
